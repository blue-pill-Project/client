  ...
  return (
    <header className="w-full flex flex-col items-center pb-2">
      ...
      <div className="w-full flex items-start justify-between mb-6 relative">
        {/* LogTimeline과 넓이를 맞추는 영역 (뒤로가기/방 정보/액션 버튼) —
            채팅하기 버튼이 flex 너비 계산에 끼어들지 않도록 접혔을 때는
            버튼을 absolute로 띄우고, 이 영역은 항상 flex-1로 꽉 채운다. */}
        <div className={`flex-1 items-start justify-between ${isChatOpen ? 'hidden lg:flex lg:px-8' : 'flex ml-4 lg:ml-8 w-full max-w-4xl'}`}>
          {/* Left: Back Button */}
          <button
        ...
        {/* ChatPanel과 넓이를 맞추는 영역 */}
        {isChatOpen ? (
          <div className="flex items-start justify-end w-full lg:w-[25vw]">
            <button
              onClick={onToggleChat}
              className="flex items-center gap-0 lg:gap-2 p-2.5 lg:px-4 lg:py-2.5 rounded-full border border-gray-700 bg-background-main transition-colors cursor-pointer text-white"
            >
              <MessageCircleMore size={18} />
              <span className="hidden lg:inline text-sm font-medium">채팅하기</span>
            </button>
          </div>
        ) : (
          <button
            onClick={onToggleChat}
            className="flex items-center gap-0 lg:gap-2 p-2.5 lg:px-4 lg:py-2.5 rounded-full border border-gray-700 bg-background-main transition-colors cursor-pointer text-gray-400 hover:text-white"
          >
            <MessageCircleMore size={18} />
            <span className="hidden lg:inline text-sm font-medium">채팅하기</span>
          </button>
